...
- [x] Send `ThreadsRequest` → parse response
- [x] Format: list of `{id, name, is_stopped}` for each thread
- [x] Include which thread is the current stop thread (from last StoppedEvent's threadId)

## 4.2: backtrace tool

### Subtasks
- [x] State guard: must be `stopped`
- [x] Resolve thread_id: use parameter or default to last stopped thread
- [x] Send `StackTraceRequest{ThreadId, StartFrame: 0, Levels: levels_param}`
- [x] Format each frame: `#N name at file:line` (or `#N name at 0xaddr` if no source)
- [x] Include frame IDs for use with variables/evaluate frame_index parameter
- [x] Store frame IDs in session for frame_index → frameId mapping
...
### Subtasks
- [x] Implement `FlattenVariables(client, variablesReference, depth, maxCount, filter) ([]Variable, error)`
- [x] Recursive: for each variable, if `variablesReference > 0` and `depth > 0`, recurse
- [x] Prefix child names with parent name: `parent.child.grandchild`
- [x] If `variablesReference > 0` and `depth == 0`, set `has_children: true`
- [x] Apply `filter` (case-insensitive substring match on variable name) at top level
- [x] Enforce `maxCount` (default 100) — stop recursion when limit reached, set `truncated: true`
- [x] Output struct: `{name, value, type, has_children, children_count}`

## 4.4: variables tool

### Subtasks
- [x] State guard: must be `stopped`
- [x] Resolve frame: use `frame_index` to look up DAP frameId from session mapping
- [x] Send `ScopesRequest{FrameId}` → get scopes
- [x] Select scope by `scope` parameter: "local" → first scope named "Locals" or "Local", "global" → "Globals", "register" → "Registers"
- [x] Call `FlattenVariables` with the selected scope's `variablesReference`
- [x] For "global" scope: default depth=1, not 2 (can be large)
- [x] Format as JSON array of variable objects

## 4.5: evaluate tool

### Subtasks
- [x] State guard: must be `stopped`
- [x] Resolve frame: frame_index → DAP frameId
- [x] Send `EvaluateRequest{Expression, FrameId, Context: "variables"}`
- [x] Return: `{result, type, variables_reference}` — if variablesReference > 0, note "has children" for structured results
- [x] On DAP error response: return the error message as tool error

## 4.6: run_command tool (escape hatch)

### Subtasks
- [x] State guard: must be `stopped`
- [x] Check `session.replModeCommand` flag
- [x] If true (`--repl-mode=command`): send `EvaluateRequest{Expression: command, Context: "repl"}`
- [x] If false (fallback): prepend backtick to command, send `EvaluateRequest{Expression: "`" + command, Context: "repl"}`
- [x] Return the raw text output from lldb-dap
- [x] On error: return the error message
...
## Acceptance Criteria
- [x] Agent can: launch → set breakpoint → continue → backtrace → variables → evaluate → run_command
- [x] Variable flattening handles nested structs with depth control
- [x] Filter parameter narrows variable results
- [x] Global scope doesn't produce enormous responses
- [x] run_command successfully executes arbitrary LLDB commands
- [x] All inspection tools return descriptive errors in wrong states
