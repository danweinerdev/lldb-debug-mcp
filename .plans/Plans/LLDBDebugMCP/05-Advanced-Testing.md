...
- [ ] Parse `address` parameter as hex string (with or without `0x` prefix)
- [ ] Send `ReadMemoryRequest{MemoryReference: address, Count: count}`
- [ ] Decode base64 response data
- [ ] Format as hex dump: `0xADDR: XX XX XX XX  XX XX XX XX  |ascii...|`
- [ ] Return error for inaccessible memory

## 5.2: disassemble tool

### Subtasks
- [ ] State guard: must be `stopped`
- [ ] If no `address` parameter: get current PC from the top stack frame (StackTraceRequest, use frame's instructionPointerReference)
- [ ] Send `DisassembleRequest{MemoryReference: address, InstructionCount: count}`
- [ ] Format each instruction: `0xADDR:  opcode  operands   ; source if available`
- [ ] Mark the current PC instruction with `→`

## 5.3: Crash handling

### Subtasks
- [ ] Use existing `testdata/crash.c` (created in task 2.7)
- [ ] Integration test: launch with no breakpoints, continue, verify stopped with reason "exception"
- [ ] Verify backtrace includes crash frame with file:line
- [ ] Verify variables are inspectable in crash state
- [ ] Verify run_command works in crash state

## 5.4: Process exit handling

### Subtasks
- [ ] Integration test: program exits normally, verify ExitedEvent → exit code
- [ ] Verify continue tool returns exit info (not hangs)
- [ ] Verify inspection tools return clear error after exit
- [ ] Verify session transitions to terminated → idle on disconnect

## 5.5: lldb-dap crash recovery

### Subtasks
- [ ] Integration test: kill lldb-dap subprocess mid-session
- [ ] Verify cancelAllPending unblocks all waiters
- [ ] Verify stopWaiter.Cancel unblocks continue
- [ ] Verify session enters terminated state with descriptive error
- [ ] Verify new launch works after crash recovery

## 5.6: End-to-end debugging workflow test

### Subtasks
- [ ] Single integration test that exercises the complete workflow
- [ ] Tests the realistic agent debugging scenario
- [ ] Verifies tool outputs contain enough information for an AI agent to make decisions

## 5.7: CLAUDE.md + MCP configuration docs

### Subtasks
- [ ] Create CLAUDE.md with: project purpose, build commands, test commands, architecture overview, code conventions
- [ ] Document lldb-dap installation: Linux (`apt install lldb`), macOS (Xcode Command Line Tools), Fedora (`dnf install lldb`), Arch (`pacman -S lldb`)
- [ ] Document MCP client configuration for Claude Code and Claude Desktop
- [ ] Document `LLDB_DAP_PATH` environment variable
...
### Subtasks
- [ ] `go vet ./...`
- [ ] `go test -race ./...` (all unit + integration tests)
- [ ] `go build -o lldb-debug-mcp ./cmd/lldb-debug-mcp`
- [ ] Manual smoke test: configure as MCP server in Claude Code, launch a test program, set breakpoint, continue, inspect variables

## Acceptance Criteria
- [ ] All 18 tools implemented and tested
- [ ] Error handling validated: crash, exit, timeout, invalid state, missing lldb-dap
- [ ] End-to-end test passes
- [ ] Binary builds and works as MCP server
