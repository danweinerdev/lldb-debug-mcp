...
func TestHandleVariablesStateGuardRejectsNonStopped(t *testing.T) {
	tests := []struct {
		name  string
		state session.State
	}{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := tools.handleVariables(context.Background(), mcp.CallToolRequest{})
			if err != nil {
				t.Fatalf("handleVariables returned error: %v", err)
			}
			if !result.IsError {
				t.Fatalf("handleVariables should return tool error when state is %s", tc.name)
			}
		})
	}
}
